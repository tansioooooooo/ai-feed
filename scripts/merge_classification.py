...
def generate_trend_reports() -> None:
    """週次・月次トレンドレポートを生成する。ANTHROPIC_API_KEY 未設定時もスキップせず実行（フォールバック動作あり）。"""
    ...
    for mode in ["weekly", "monthly"]:
        try:
            ...
            if result.returncode != 0 and result.stderr:
                print(f"  trend report ({mode}) warning: {result.stderr.rstrip()}")
        except Exception as e:
            print(f"  trend report ({mode}) failed: {e}")
